...
class AgriDataProcessor:
    def __init__(self, kb_path: str = "./", output_path: str = "./dist"):
    ...
    def process_pdf_documents(self, pdf_dir: str = "data") -> Dict[str, Any]:
        """Process PDF documents and extract text content"""
        pdf_path = self.kb_path / pdf_dir
        if not pdf_path.exists():
            pdf_path = self.kb_path  # Fallback to kb_path if pdf_dir doesn't exist
...
def main():
    ...
    processor = AgriDataProcessor(kb_path=args.input_dir, output_path=args.output_dir)
